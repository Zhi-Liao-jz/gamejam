...
var _sd_room: int = -1
var _heater_overheat: bool = false
var _heater_burned: bool = false
...
var _summary_buttons: HBoxContainer = null  # 结算面板底部按钮栏（商店 / 下一天 / 重试）
...
func _draw() -> void:
	...
	for i: int in _room_count():
		...
		if i == _heater_room and _heater_burned:
...
func _poll_heater() -> void:
	var heater := get_tree().get_first_node_in_group("heater") as Heater
	if heater != null:
		_heater_overheat = heater.has_overheat()
		_heater_burned = heater.has_burned()
		_heater_room = heater.room_id
	else:
		_heater_overheat = false
		_heater_burned = false
...
## 结算面板底部按钮栏：通关显示 [商店][下一天]，失败显示 [重试本日]。
## 按钮通过 EventBus 把选择转给当前 DayManager 状态（Settlement / Failed）处理。
func _build_summary_buttons() -> void:
	...
	_next_button = _make_summary_button("下一天 ▶", "settlement_next")
	_retry_button = _make_summary_button("重试本日", "settlement_retry")


func _make_summary_button(text: String, event: String) -> Button:
	var button := Button.new()
	button.text = text
	button.custom_minimum_size = Vector2(150.0, 40.0)
	button.pressed.connect(_on_summary_button.bind(event))
	_summary_buttons.add_child(button)
	return button


func _on_summary_button(event: String) -> void:
