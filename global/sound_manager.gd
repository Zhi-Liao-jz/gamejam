...
func play(sound_key: String, randomize_pitch: bool = false, prevent_overlap: bool = true) -> void :
	if not SOUNDS.has(sound_key):
		...
		return
	if prevent_overlap and _is_on_cooldown(sound_key): return
...
func play_at(sound_key: String, source_node: Node2D, randomize_pitch: bool = false, prevent_overlap: bool = true) -> void :
	if not is_instance_valid(source_node): return
	if not _is_on_screen(source_node): return
...
func sfx_pitched(sound_key: String, pitch: float, prevent_overlap: bool = true) -> void :
	if not SOUNDS.has(sound_key): return
	if prevent_overlap and _is_on_cooldown(sound_key): return
	var path: String = _pick_path(SOUNDS[sound_key])
	if path.is_empty(): return
	var stream = _load_stream(path)
...
func play_delayed(sound_key: String, delay: float, randomize_pitch: bool = false, prevent_overlap: bool = true) -> void :
